...
// ─────────────────────────────────────────────────────────────────────────────
// LIVE MATCHER  — mirrors reference LiveMatcher
// ─────────────────────────────────────────────────────────────────────────────
...
class TemplateProcessor {
  ...
  async processVideo(file: Blob, onProgress: (p: number) => void): Promise<{ series: NormFrame[]; duration: number }> {
    this.series = [];
    await this.initPose();
  ...
  private sendFrame(canvas: HTMLCanvasElement): Promise<void> {
    return new Promise(res => {
      ...
      const to = setTimeout(() => { if(done)return; done=true; this.frameResolved=null; res(); }, 3000);
      this.pose!.send({ image: canvas }).catch(() => { if(!done){done=true;clearTimeout(to);this.frameResolved=null;res();} });
    });
  }
}
